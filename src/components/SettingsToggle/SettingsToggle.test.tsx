...
const progressDisabledRef: { current: boolean } = { current: false };
const setProgressDisabled = vi.fn();

vi.mock('@/components/ProgressModeProvider', () => ({
  useProgressMode: () => ({
    disabled: progressDisabledRef.current,
    setDisabled: setProgressDisabled,
  }),
}));

// `true` → progress is empty (no completed lessons, no furthest pointer), so
// enabling free reading needs no confirmation; `false` → non-empty.
const progressEmptyRef: { current: boolean } = { current: true };

vi.mock('@/lib/progress', () => ({
  getProgress: () => ({}),
  getCompletedCount: () => (progressEmptyRef.current ? 0 : 1),
  getFurthestKey: () => (progressEmptyRef.current ? null : 'module-1/lesson-1'),
}));
...
beforeEach(() => {
  ...
  progressEmptyRef.current = true;
  setProgressDisabled.mockReset();
  ctxRef.current = {
    prefs: { proseSize: 1, codeSize: 0, proseFont: 'serif', codeFont: 'jetbrains' },
  };
...
function freeReadingToggle(): HTMLButtonElement {
  const node = container.querySelector<HTMLButtonElement>('button[data-free-reading-toggle]');
  if (!node) throw new Error('free-reading toggle not rendered');
  return node;
}
...
describe('SettingsToggle', () => {
  ...
  it('renders the free-reading switch off when progress is enabled', () => {
    progressDisabledRef.current = false;
    renderToggle();
    click(trigger());
    expect(freeReadingToggle().getAttribute('aria-checked')).toBe('false');
    expect(freeReadingToggle().getAttribute('data-active')).toBe('false');
  });

  it('renders the free-reading switch on when progress is disabled', () => {
    progressDisabledRef.current = true;
    renderToggle();
    click(trigger());
    expect(freeReadingToggle().getAttribute('aria-checked')).toBe('true');
    expect(freeReadingToggle().getAttribute('data-active')).toBe('true');
  });

  it('server-renders the free-reading switch off even when free reading is active', () => {
    // The SSR pass has no access to localStorage or the <html> attribute, so its
    // markup must show the switch OFF regardless of the live flag. If the client
    // seeded the switch ON during hydration, React 18 would keep the server
    // attribute on the mismatch and the toggle would look inactive after a
    // reload. Locking the SSR output to `false` guards the mounted-gated render
    // that prevents that hydration mismatch.
    progressDisabledRef.current = true;
    const probe = document.createElement('div');
    probe.innerHTML = renderToStaticMarkup(<SettingsToggle />);
    const toggle = probe.querySelector('button[data-free-reading-toggle]');
    expect(toggle).not.toBeNull();
    expect(toggle!.getAttribute('aria-checked')).toBe('false');
    expect(toggle!.getAttribute('data-active')).toBe('false');
  });

  it('enables free reading immediately when progress is empty', () => {
    progressDisabledRef.current = false;
    progressEmptyRef.current = true;
    renderToggle();
    click(trigger());
    click(freeReadingToggle());
    expect(setProgressDisabled).toHaveBeenCalledTimes(1);
    expect(setProgressDisabled).toHaveBeenCalledWith(true);
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).toBeNull();
  });

  it('asks for confirmation before enabling free reading with non-empty progress', () => {
    progressDisabledRef.current = false;
    progressEmptyRef.current = false;
    renderToggle();
    click(trigger());
    click(freeReadingToggle());
    expect(setProgressDisabled).not.toHaveBeenCalled();
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).not.toBeNull();
    click(container.querySelector<HTMLButtonElement>('[data-free-reading-confirm]')!);
    expect(setProgressDisabled).toHaveBeenCalledTimes(1);
    expect(setProgressDisabled).toHaveBeenCalledWith(true);
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).toBeNull();
  });

  it('cancelling the confirmation does not disable progress', () => {
    progressDisabledRef.current = false;
    progressEmptyRef.current = false;
    renderToggle();
    click(trigger());
    click(freeReadingToggle());
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).not.toBeNull();
    click(container.querySelector<HTMLButtonElement>('[data-free-reading-cancel]')!);
    expect(setProgressDisabled).not.toHaveBeenCalled();
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).toBeNull();
  });

  it('drops a pending reset confirmation when the popover closes and stays gone on reopen', () => {
    progressDisabledRef.current = false;
    progressEmptyRef.current = false;
    renderToggle();
    click(trigger());
    click(freeReadingToggle());
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).not.toBeNull();
    // Close without choosing — the destructive prompt must not survive.
    pressKey('Escape');
    expect(popover().hasAttribute('hidden')).toBe(true);
    click(trigger());
    expect(popover().hasAttribute('hidden')).toBe(false);
  ...
  it('disables free reading without confirmation when turning it off', () => {
    progressDisabledRef.current = true;
    progressEmptyRef.current = false;
    renderToggle();
    click(trigger());
    click(freeReadingToggle());
    expect(setProgressDisabled).toHaveBeenCalledTimes(1);
    expect(setProgressDisabled).toHaveBeenCalledWith(false);
    expect(container.querySelector('[data-free-reading-confirm-prompt]')).toBeNull();
  });
});
