...
describe('clearProgress', () => {
  it('removes both the progress map and the furthest pointer', () => {
    const course = loadCourse();
    markCompletedAndAdvance(course, lessonKey('01-foo', '01-02-deep'));
    expect(window.localStorage.getItem(PROGRESS_STORAGE_KEY)).not.toBeNull();
    expect(window.localStorage.getItem(FURTHEST_STORAGE_KEY)).not.toBeNull();

    clearProgress();

    expect(window.localStorage.getItem(PROGRESS_STORAGE_KEY)).toBeNull();
    expect(window.localStorage.getItem(FURTHEST_STORAGE_KEY)).toBeNull();
    expect(getProgress()).toEqual({});
    expect(getFurthestKey()).toBeNull();
  });

  it('dispatches a progress-change event', () => {
    const listener = vi.fn();
    window.addEventListener(PROGRESS_CHANGE_EVENT, listener);
    try {
      clearProgress();
  ...
  it('is safe when localStorage throws (storage unavailable)', () => {
