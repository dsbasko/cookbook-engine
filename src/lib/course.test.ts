...
brand:
  ...
  logo: /logo.svg
  glyph: "P"
  siteUrl: https://postgres.dsbasko.dev
  level: SQL
  ...
  hero:
    ...
    tail: { ru: "пишет SQL", en: "write SQL" }
  ogImage:
    title: { ru: "PostgreSQL Cookbook", en: "PostgreSQL Cookbook" }
    subtitle: { ru: "Курс по PostgreSQL", en: "A PostgreSQL course" }
    footerTag: "PostgreSQL · SQL"
    alt: { ru: "PostgreSQL Cookbook — курс", en: "PostgreSQL Cookbook — course" }
modules:
  - id: 01-foo
    title: Foo
    description: Foo
...
describe('parseCourse — brand parsing', () => {
  it('parses the full brand section with scalar fields', () => {
    const course = parseCourse(BRAND_YAML);
    expect(course.brand?.accent).toBe('#336791');
    expect(course.brand?.accentDark).toBe('#4a8fc7');
    expect(course.brand?.logo).toBe('/logo.svg');
    expect(course.brand?.glyph).toBe('P');
    expect(course.brand?.siteUrl).toBe('https://postgres.dsbasko.dev');
    expect(course.brand?.level).toBe('SQL');
  });

  it('resolves localized brand fields for lang=ru', () => {
    const course = parseCourse(BRAND_YAML, 'ru');
    expect(course.brand?.breadcrumbRoot).toBe('PostgreSQL Cookbook');
    expect(course.brand?.hero).toEqual({
      lead: 'PostgreSQL',
      accent: 'для тех, кто',
      tail: 'пишет SQL',
    });
    expect(course.brand?.ogImage?.subtitle).toBe('Курс по PostgreSQL');
    expect(course.brand?.ogImage?.alt).toBe('PostgreSQL Cookbook — курс');
  });

  it('resolves localized brand fields for lang=en', () => {
    const course = parseCourse(BRAND_YAML, 'en');
    expect(course.brand?.hero?.accent).toBe('for people who');
    expect(course.brand?.hero?.tail).toBe('write SQL');
    expect(course.brand?.ogImage?.subtitle).toBe('A PostgreSQL course');
    expect(course.brand?.ogImage?.footerTag).toBe('PostgreSQL · SQL');
  });

  it('omits brand entirely when the section is absent (backward compat)', () => {
    const course = parseCourse(MIN_VALID_YAML);
    expect(course.brand).toBeUndefined();
  });

  it('accepts an empty brand mapping', () => {
    const course = parseCourse(`${MIN_VALID_YAML}brand: {}\n`);
    expect(course.brand).toEqual({});
  });

  it('throws on a non-hex accent', () => {
    expect(() =>
      parseCourse(`${MIN_VALID_YAML}brand:\n  accent: "blue"\n`),
    ).toThrow(/brand\.accent .* hex colour/);
  });

  it('throws on a non-http siteUrl', () => {
    expect(() =>
      parseCourse(`${MIN_VALID_YAML}brand:\n  siteUrl: "ftp://example.com"\n`),
    ).toThrow(/brand\.siteUrl .* http\(s\) URL/);
  });

  it('throws when siteUrl is not a URL at all', () => {
    expect(() =>
      parseCourse(`${MIN_VALID_YAML}brand:\n  siteUrl: "not a url"\n`),
    ).toThrow(/brand\.siteUrl .* valid URL/);
  });

  it('throws when hero is missing a required part', () => {
    const yaml = `${MIN_VALID_YAML}brand:\n  hero:\n    lead: A\n    accent: B\n`;
    expect(() => parseCourse(yaml)).toThrow(/brand\.hero\.tail/);
  });

  it('throws when brand is not a mapping', () => {
    expect(() => parseCourse(`${MIN_VALID_YAML}brand: "nope"\n`)).toThrow(
      /brand must be a mapping/,
    );
  });
});

describe('brand resolvers — defaults vs overrides', () => {
  it('falls back to Kafka defaults when no brand is set', () => {
    const course = parseCourse(MIN_VALID_YAML);
    expect(resolveBrandName(course)).toBe('Kafka Cookbook');
    expect(resolveBrandGlyph(course)).toBe('K');
    expect(resolveBrandLevel(course)).toBe('Go');
    expect(resolveOgFooterTag(course)).toBe('Apache Kafka · Go');
  });

  it('returns brand values when present', () => {
    const course = parseCourse(BRAND_YAML, 'en');
    expect(resolveBrandName(course)).toBe('PostgreSQL Cookbook');
    expect(resolveBrandGlyph(course)).toBe('P');
    expect(resolveBrandLevel(course)).toBe('SQL');
    expect(resolveOgFooterTag(course)).toBe('PostgreSQL · SQL');
  });
});

describe('buildBrandAccentCss — accent cascade override', () => {
  it('returns null when no accent is configured', () => {
    const course = parseCourse(MIN_VALID_YAML);
    expect(buildBrandAccentCss(course.brand)).toBeNull();
    expect(buildBrandAccentCss(undefined)).toBeNull();
  });

  it('emits high-specificity overrides for both light and dark', () => {
    const css = buildBrandAccentCss(parseCourse(BRAND_YAML).brand) ?? '';
    // `:root[data-theme=…]` (0,1,1) out-specifies tokens.css `[data-theme=…]`
    // (0,1,0), so the brand colour wins regardless of stylesheet order.
    expect(css).toContain(":root[data-theme='light']");
    expect(css).toContain(":root[data-theme='paper']");
    expect(css).toContain(":root[data-theme='dark']");
    // Light/paper use accent; dark uses accentDark.
    expect(css).toContain('--accent-main:#336791;');
    expect(css).toContain('--accent-main:#4a8fc7;');
    // Family is re-derived from the base, not left at the kafka defaults.
    expect(css).toContain('--accent-main-hover:color-mix');
    expect(css).toContain('--accent-main-soft:color-mix');
  });

  it('falls back to the light accent for dark when accentDark is absent', () => {
    const course = parseCourse(`${MIN_VALID_YAML}brand:\n  accent: "#abcdef"\n`);
    const css = buildBrandAccentCss(course.brand) ?? '';
    expect(css).toContain(":root[data-theme='dark']{--accent-base:#abcdef;");
  });
});
