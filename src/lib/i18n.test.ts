...
describe('brand-neutral defaults', () => {
  it('drops the brand-baked notFoundMetadataTitle key (composed at call site)', () => {
    expect('notFoundMetadataTitle' in UI_STRINGS.ru).toBe(false);
    expect('notFoundMetadataTitle' in UI_STRINGS.en).toBe(false);
  });

  it('keeps the reading-prefs preview prose free of course-specific branding', () => {
    for (const lang of LANGS) {
      const preview = UI_STRINGS[lang].readingPrefsPreviewProse;
      expect(preview).not.toMatch(/kafka|apache/i);
    }
  });

  it('still ships the Kafka hero defaults (overridable via brand.hero)', () => {
    expect(UI_STRINGS.ru.heroTitleLead).toBe('Kafka');
    expect(UI_STRINGS.en.heroTitleLead).toBe('Kafka');
  });
});
